...
public class TeeAndLoggingTestCase {
	...
	@Ignore @Test public void testTwoTeeesAndALogger() {
		...
		try {
			...
			// The package java.util.logging conflicts with a package accessible
			// from another module: java.logging ConsoleHandler.java
			// /code35/src/java/util/logging line 1 Java Problem
			...
			// logger.addHandler(handler);
			// logger.info("logger 1");
			String expected=tee.prefix+string1+lineSeparator+tee.prefix+string2+lineSeparator+tee2.prefix+string3+lineSeparator+tee2.prefix+string4
					+lineSeparator+"00000         main    INFO                         logger 0 in io.TeeAndLoggingTestCase.testTwoTeeesAndALogger()"+"\n";
			// no ctlf after log messages!
			// +lineSeparator;
	...
	@Test public void testAddSysoutAndSyserrToPrintStreamOfTee() throws InterruptedException {
		...
		// tee.addOutputStream(console.out); // extra because tee is based on
		// console out
		tee.addOutputStream(console.err);
