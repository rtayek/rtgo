#!/bin/bash
# Adrian Dvergsdal https://github.com/atmoz
#
# NAME
...
#javac --enable-preview --source 17 -cp "bin ${pathArray[@]} && exec java -cp bin:$CLASSPATH org.junit.runner.JUnitCore ${classArray[@]}
echo "classpath $CLASSPATH"
set -x
echo "pathArray: ${pathArray[@]}"
echo "classArray: ${classArray[@]}"
#javac --enable-preview --source 17 -cp "bin;$CLASSPATH" ${pathArray[@]}
javac --enable-preview --source 17 -cp "bin;${pathArray[@]}" && exec java --enable-preview --source 17 -cp "bin;$CLASSPATH" org.junit.runner.JUnitCore ${classArray[@]}
