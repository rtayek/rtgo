...
public class Model extends Observable { // model of a go game or problem forrest
	...
	public enum Where { // generalize to not turn or not playing this color?
		...
		public static Where isPoint(Board board,int band,Point2D.Float point,Point closest) {
			if(point!=null) {
				...
				if(distance>=epsilon) return notCloseEnough;
			}
	...
	public static File insureExtension(File file,String desiredExtension) {
		...
		if(extension==null||!extension.equalsIgnoreCase("sgf")) file=new File(file.getParent(),file.getName()+"."+desiredExtension);
		return file;
	}
	void checkForLittleGolem(MNode node) {
		boolean isTorus=false;
		for(SgfProperty property:node.sgfProperties())
			for(String string:property.list())
	...
	public void setBoard(Board board) { // this is wierd! - do a notify or
										// something!!!
	...
	public void setBoardTopology(Topology type) {
		state.topology=type;
	}
	...
	public void setBand(int band) {
		Logging.mainLogger.warning("set band to: "+band);
		state.band=band;
	}
	public int band() {
		return state.band;
	}
	public synchronized int moves() {
		return state.moves;
	}
	...
	public void makeMoves(List<Move2> moves) {
	...
	public Point generateRandomMove() {
		...
		for(;board().at(point).equals(Stone.vacant);point=new Point(random.nextInt(board().width()),random.nextInt(board().depth())),++i) {
			Move2 move=new Move2(MoveType.move,turn(),point);
			if(move(move)==MoveResult.legal) break;
			else if(i>limit) {
				Logging.mainLogger.warning("break out "+i+" "+limit);
				point=null;
				break;
			}
		}
	...
	public static void generateSillyMoves(Model model,int moves) { // like silly
		...
		int n=0;
		for(Move2 move=model.generateSillyMove(model.turn());move!=null;move=model.generateSillyMove(model.turn()),turn=turn.otherColor(),++n) {
			if(n>=moves) break;
			if(n>=model.board().width()*model.board().depth()) break;
			model.move(move);
		}
	}
	public static void generateAndMakeMoves(Model model,int moves) { // like
																		// silly
																		// (almost)
		if(moves==0) return;
		int n=0;
	...
	public static List<Point> generateRandomMovesInList(Model model,int n) {
		...
		for(Point point=model.generateRandomMove();point!=null;points.add(point),point=model.generateRandomMove(),turn=turn.otherColor()) {
			if(model.moves()>=n) break;
	...
	void setApplicationFromSgf(String app) {
		state.application=app;
	}
	void setSgfVersion(String version) {
		state.sgfVersion=version;
	}
	...
	void sgfMakeMoveFromProperty(Stone color,Point point) {
	...
	public MoveResult moveAndPlaySound(Stone color,Point point) {
		...
		if(ok) {
			...
			if(wasLegal==MoveResult.legal) {
				...
				if(state.capturedBlocks!=null&&state.capturedBlocks.size()>0||state.selfCaptured!=null&&!state.selfCaptured.points().isEmpty())
			...
			return MoveResult.badRole; // not really illegal, it's a role
										// violation
	...
	public int playOneMove(Move2 move) {
		...
		if(move.equals(Move2.nullMove)) throw new RuntimeException(move+" 1 "+lastMoveGTP()+" "+lastMove2()+" move oops");
		int moves=moves();
	...
	public int playOneMove(Stone color,String gtpMoveString) {
		...
		if(move.equals(Move2.nullMove)) { throw new RuntimeException(move+" "+lastMoveGTP()+" "+lastMove2()+" 6 move oops"); }
	...
	public MoveResult move(Move2 move) {
		...
		try {
			...
			if(wasLegal!=MoveResult.legal) Logging.mainLogger.info(name+" not legal because: "+wasLegal+" "+turn()+" at move #"+moves()
	...
	public Where isPoint(Point point) {
		return Where.isPoint(board(),band(),null,point);
	}
	MoveResult addMoveNodeAndExecute(Move2 move) {
		...
		Point point=move.point;
		if(point==null) {
			Logging.mainLogger.fine("point is null!");
		}
		if(currentNode()!=null) {
			...
			if(where.equals(Model.Where.onVacant)) {
				if(color.equals(turn())||role.equals(Role.anything)) {
					...
					if(!isduplicateHash()) {
						...
						delete();
						if(!checkingForLegalMove) setChangedAndNotify(new Event.Hint(Event.illegalMove,"duplicate hash"));
	...
	private void savethisprocessProperty(SgfProperty property) {
		...
		if(p2!=null) {
			switch(p2) {
				...
				// vs list of points
				// ;FF[4]GM[1]AP[RTGO]
				// SZ[19:17]KM[4.5]RG[ij][jj][kj][ii][ji][ki][ih][jh][kh]
				case AB:
				case AW:
				...
				case W:
					...
					ensureBoard();
					if(board().width()<=Board.standard&&board().depth()<=Board.standard&&string.equals("tt")) isPass=true; // hack
																															// for
																															// some
																															// sgf
																															// wierdness/
				...
				case C:
					...
					if(comment.startsWith(sgfApplicationName)) { // get type and
																	// shape
				...
				case RG:
					...
					// maybe restrict diamond to normal for now and add some
					// holes later?
					ensureBoard();
	...
	public static Collection<SgfNode> mainLineFromCurrentPosition(Model model) {
		...
		try {
			while(Navigate.down.canDo(model)) {
	...
	public static class State implements Cloneable { // needs an equals or
														// isEqual method.
		public State() {
		...
		private void adjustPrisonerCounts(List<Block> captured,int sign) {
			if(captured!=null) for(Block block:captured)
				adjustPrisonerCount(block,sign);
		}
		...
		public void sgfMakeMove(Stone stone,Point point) {
			...
			hash=hash(board);
			aSingleStoneWasCaptured=capturedBlocks!=null&&capturedBlocks.size()==1&&capturedBlocks.get(0).points().size()==1;
			moves++;
			toggleTurn();
		}
		private void sgfUnmakeMove(Point at) {
			if(board==null) {
				Logging.mainLogger.warning(""+" "+"board is null in unmake!");
				return;
			}
	...
	static { // https://chessprogramming.wikispaces.com/Transposition+Table#KeyCollisions
		Random random=new Random();
		for(int i=0;i<LargestBoardSize;i++)
			for(int j=0;j<LargestBoardSize;j++) {
