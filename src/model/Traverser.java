...
import static sgf.Parser.sgfPath;
import java.io.*;
...
class Traverser { // half of a visitor
    ...
    public static void main(String[] args) {
        ...
        SgfNode games_=ModelIo.restoreSGF(FileIO.toReader(expectedSgf));
        ...
        games_=ModelIo.restoreSGF(FileIO.toReader(file));
        games=MNode.toGeneralTree(games_);
        traverser.visit(games);
        Logging.mainLogger.info(""+" "+"|||");
        expectedSgf=getSgfData("oneMoveAtA1");
        games_=ModelIo.restoreSGF(FileIO.toReader(expectedSgf));
        games=MNode.toGeneralTree(games_);
        traverser.visit(games);
        Logging.mainLogger.info(""+" "+"|||");
