...
public interface Tree {
	public static class BinaryNode<T> {
		...
		GeneralNodee<T> convert() {
			return null;
		}
		BinaryNode(BinaryNode<T> left,BinaryNode<T> right,T t) {
			this.left=left;
			this.right=right;
			this.t=t;
		}
		...
		private BinaryNode<T> lastChild() {
			return BinaryTreeSupport.lastChild(left,n->n.right);
		}
		private void addDescendant(T t) {
			BinaryTreeSupport.appendDescendant(this,n->n.left,(parent,child)->parent.left=child,new BinaryNode<T>(null,null,t));
		}
	...
	public static class GeneralNodee<T> /*extends BinaryNode<T>*/ {
		...
		BinaryNode<T> convert() {
			return null;
		}
		final T t;
		public final ArrayList<T> children=new ArrayList<>();
	}
	public static class Sgf extends ArrayList<SgfProperty> { // will be data in
																// trees now
		public Sgf() {}
		public Sgf(List<SgfProperty> properties) {
			this();
			addAll(properties);
		}
		...
		public void setIsAMoveFlags() {
			// http://www.red-bean.com/sgf/user_guide/index.html#move_vs_place
			// says: Therefore it's illegal to mix setup properties and move
			// properties within the same node.
			for(SgfProperty property:this) {
				if(property.p() instanceof Setup) hasASetupType=true;
				if(property.p() instanceof sgf.Move) {
					hasAMoveType=true;
				}
		...
		@Override public String toString() {
			...
			for(Iterator<SgfProperty> i=iterator();i.hasNext();)
