...
public class Parser {
	...
	private List<String> cValues() throws IOException {
		...
		for(;nextIsDelimiter()&&lastDelimiter==startCValueCharacter;) {
			...
			token=getToken();
			if(token.charAt(0)!=endCValueCharacter||token.length()!=1) throw new RuntimeException("missing "+endCValueCharacter);
			list.add(value);
		}
		return list;
	}
	private SgfProperty getProperty() throws IOException {
		String id=getToken();
		if(isDelimiter(id.charAt(0))) {
			reader.unread(id.charAt(0));
			return null;
		}
		P p=P.idToP.get(id);
		if(p==null) {
	...
	private SgfNode parse() throws IOException { // recursive
		...
		// parserLogger.info("added sequence:
	...
	boolean endOfFile;
	int line;
	public static final char startGameCharacter='(',endGameCharacter=')',startNodeCharacter=';',startCValueCharacter='[',endCValueCharacter=']';
	public static final String startGameString=startGameCharacter+"";
	public static final char[] delimiter= {startGameCharacter,endGameCharacter,startNodeCharacter,startCValueCharacter,endCValueCharacter};
	...
	static final Set<File> badSgfFiles=new LinkedHashSet<>();
	static {
		badSgfFiles.add(new File("sgf/old/annotated/test1.sgf")); // missing
																	// endGameCharacter)
		...
		badSgfFiles.add(new File("strangesgf/Comments/escaped and not escaped/10744 esc.Zeichen nicht esc - gefangen.sgf"));
		...
		badSgfFiles.add(new File(exception,"repaired damaged file 1,  comment = 63193 x b-slash (too much example code).sgf"));
		badSgfFiles.add(new File("strangesgf/FileSize, too long Strings/11478 Code chopped apart - missing ]))), many b-slashes in unfinished C.sgf"));
