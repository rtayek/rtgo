...
abstract class WidgetOptions extends OptionsABC implements Widgets {
	public abstract class WidgetOption<T extends Enum<T>,R extends Comparable<R>>extends Option<T,R> implements Widget {
		public WidgetOption(T t,Object defaultValue,Range<R> range) {
...
public class SpinnerOptions extends WidgetOptions {
	public class SpinnerOption<T extends Enum<T>,R extends Comparable<R>>extends WidgetOption<T,R> {
		public SpinnerOption(T t,Object defaultValue,Range<R> range,JSpinner jSpinner,String tooltipText,KeyStroke keyStroke) {
			super(t,defaultValue,range);
		...
		public void enableButton(boolean enable) {
			jSpinner.setEnabled(enable);
		}
		public boolean isEnabled() {
			return jSpinner.isEnabled();
		}
		@Override public boolean isValueInWidget(Object value) { // ok - all the
																	// same
		...
		@Override public boolean setValueInWidget(Object value) {
			if(value==null) {
				Logging.mainLogger.warning("null value for parameter: "+t);
				return false;
			}
		...
		@Override public boolean setValueInWidgetFromCurrentValue() { // for
																		// this
																		// parameter
			boolean rc=false;
	...
	public void enableAll(Mediator mediator) { // default behavious is to enable
												// all of the buttons.
		for(Option<?,?> button:options())
			((SpinnerOption<?,?>)button).jSpinner.setEnabled(true);
...
class SpinnerParameterOptions extends SpinnerOptions {
	...
	private <R extends Comparable<R>> void addParameterSpinnerOption(Parameters parameter,List<R> values,int width) {
		new SpinnerOption<Parameters,R>(parameter,parameter.defaultValue,(Range<R>)null,spinner(values,width),(String)null,(KeyStroke)null) {
			@Override public Object fromString(String string) {
				return parameter.fromString(string);
			}
	...
	// how much of the above code can we push up into the abc or the options
	// class?
	// the above is old.
