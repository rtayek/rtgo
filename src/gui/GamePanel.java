...
import sgf.*;
class Line {
	Line(Point p1,Point p2,double width) {
		this.p1=p1;
		this.p2=p2;
		this.width=width;
	}
...
@SuppressWarnings("serial") class SgfNodePanel extends JPanel { // not used yet
	SgfNodePanel(Mediator mediator) {
...
class MyShowingListener implements HierarchyListener {
	...
	public MyShowingListener(JComponent jc) {
		component=jc;
	}
...
public class GamePanel extends JPanel {
	...
	private MNode isAlreadyInTree(Point closest) {
		...
		for(MNode child:node.children())
			for(SgfProperty property:child.sgfProperties())
				if(mediator.model.turn().equals(Stone.black)&&property.p().equals(P.B)||mediator.model.turn().equals(Stone.white)&&property.p().equals(P.W)) {
	...
	void processClick(MouseEvent e) {
		...
		if(SwingUtilities.isLeftMouseButton(e)) {
			if(board==null) {
				Logging.mainLogger.info(mediator.model.name+" "+"ignoring left click");
				return;
			}
	...
	private double line(Point p1,Point p2,double x) {
		return p1.y+(p2.y-p1.y)/(p2.x-p1.x)*x;
	}
	...
	private void normalLines(final int x0,final int y0) {
		for(int y=0;y<board.depth();y++)
			addLine(x0,y0+y*dy,x0+(board.width()-1)*dx,y0+y*dy);
		for(int x=0;x<board.width();x++)
	...
	void initialize(double boardHeightInPixels) {
		...
		if(board.topology()==Topology.torus) { // some problem here with band=7
												// and
			// width=depth=11
	...
	private void paintBand(Graphics g,int x,int y,Stone stone,boolean paintMoveNumber) {
		...
		if(0<=y&&y<band) {
			...
			if(board.width()-band<=x&&x<board.width()) paintBandStone(g,stone,index,x-board.width(),y+board.depth(),paintMoveNumber);
		}
	...
	private void paintStone(Graphics g,int x,int y) {
		...
		if(!stone.equals(Stone.vacant)) {
			...
			if(board.topology().equals(Topology.torus)) paintBand(g,xO,yO,stone,new Point(x,y).equals(mediator.model.lastMoveGTP()));
		}
	}
	@Override public void paintComponent(Graphics g) {
		super.paintComponent(g);
		if(board==null) {
			...
			if(board.starPoints()!=null) for(int i=0;i<board.starPoints().size();i++) {
				...
				if(!board.at(point).equals(Stone.edge)) g.fillOval(pUpperLeft.x+point.x*dx-dx-dx/8,pUpperLeft.y+point.y*dy-dy-dy/8,dx/4,dy/4);
			}
	...
	private void drawStatus(Graphics g,int x,int y) {
		...
		String string=""+mediator.model.prisoners(Stone.black)+" captured black stones"+", "+mediator.model.prisoners(Stone.white)+" captured white stones"+", "
				+mediator.model.komi()+" komi"+", "+mediator.model.moves()+" moves.";
