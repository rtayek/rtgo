...
class Mediator implements Observer,ActionListener {
	...
	Mediator(Model model,Main main,TextView textView) {
		...
		if(useKeys) main.addKeyListener(new KeyListener() {
	...
	private void bindMenuOpenShortcut(JMenuBar menuBar,JMenu menu,int keyCode) {
		String actionKey="open-menu-"+keyCode;
				...
				if(hint instanceof Event||hint instanceof Event.Hint) {
					...
					switch(event) {
						...
						case illegalMove:
							...
							ButtonsABC<SouthPanels.MyEnums>.ButtonWithEnum scroll=newSouthPanel.buttons.get(SouthPanels.MyEnums.scroll);
	...
	void initializeGamePanel() { // what is this doing?
		...
		if(tryNew) {
			...
			Logging.mainLogger.info(model.name+" "+"re initializing game panel with board "+id+" "+model.board().topology());
