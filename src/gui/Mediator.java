...
class Mediator implements Observer, ActionListener {
	...
	Mediator(Model model, Main main, TextView textView) {
		...
		Logging.mainLogger
				.info(model.name + " width=" + width + ", height=" + height + ", pixels=" + boardHeightInPixels);
		JMenuBar jMenuBar = createMenuBar();
		if (main.isApplet())
			main.applet().setJMenuBar(jMenuBar);
		else
			main.frame().setJMenuBar(jMenuBar);
		...
		if (old != null) {
			System.out.println("removing: " + old.getName());
			main.remove(old);
			main.validate();
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("Open ...")) {
			try {
				JFileChooser fileChoser = new JFileChooser(
						lastLoadDirectory != null ? lastLoadDirectory : new File("."));
			...
			try {
				JFileChooser fileChoser = new JFileChooser(
						lastSaveDirectory != null ? lastSaveDirectory : new File("."));
				fileChoser.setFileFilter(new FileNameExtensionFilter("SGF file", "sgf"));
				// fileChoser.s
		...
		// and here also?
			...
			Toast.toast("Topological GO (alpha)");
	}
	...
	public void update(Observable observable, Object hint) {
		...
		if (observable instanceof Model) {
			...
			if (model == this.model) {
				...
				if (model.board() != null)
					initializeGamePanel(); // gross overkill
				// maybe just return here and see what happens?
				else
					Logging.mainLogger.fine(model.name + " " + "board is null, skipping initializeGamePanel()");
				if (hint instanceof Event || hint instanceof Event.Hint) {
					Event event = null;
					if (hint instanceof Event)
						event = (Event) hint;
				...
				if (useNewWestPanel) {
					...
					westPanel.setPlayerColor();
				if (gamePanel != null) {
					Logging.mainLogger.fine(model.name + " " + "request repaint");
					gamePanel.repaint();
				} else
					Logging.mainLogger.warning(model.name + " " + "no game panel to update");
			} else
				throw new RuntimeException("not our model!");
		} else
	...
	private void addGamePanels() {
		...
		if (!main.isApplet())
			...
			; // https://community.oracle.com/thread/1294964
	}
	...
	void initializeGamePanel() { // what is this doing?
		...
		if (tryNew) {
			...
			Board b = model.board();
			if (b == null)
				model.setBoard(b = Board.factory.create());
			...
			Logging.mainLogger.info(
					model.name + " " + "re initializing game panel with board " + id + " " + model.board().topology());
			if (true)
			...
			if (gamePanel != null) {
				System.out.println("removing: " + gamePanel.getName());
