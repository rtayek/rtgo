...
import java.util.*;
import javax.swing.*;
public abstract class ButtonsABC<T extends Enum<?>> {
    ...
    public class ButtonWithEnum /*extends AbstractButton */ {
        ...
        public ButtonWithEnum(T t,AbstractButton abstractButton,String tooltipText) { this(t,abstractButton,tooltipText,null); }
    ...
    public ButtonWithEnum get(T e) { return map.get(e); }
    public Set<T> enums() { return map.keySet(); }
    ...
    public T valueOf(String name) { for(T e:enums()) if(e.name().equals(name)) return e; return null; }
    public <E extends Enum<?>> void add(Class<E> clazz) {
        E[] enums=clazz.getEnumConstants();
        if(enums!=null&&enums.length>0) for(E e:enums) new ButtonWithEnum((T)e);
        else throw new RuntimeException(clazz+" has no enums!");
    }
    private final Map<T,ButtonWithEnum> map=new LinkedHashMap<>();
