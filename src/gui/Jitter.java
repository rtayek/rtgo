...
public class Jitter {
	static class Zero extends Jitter {
		...
		@Override int xJitter(int index) {
			return 0;
		}
		@Override int yJitter(int index) {
			return 0;
		}
	}
	...
	int xJitter(int index) {
		return xJitters[index];
	}
	int yJitter(int index) {
		return yJitters[index];
	}
	...
	static Jitter get(int width,int depth) {
		...
		if(jitter==null) {
			jitter=new Jitter(point.x,point.y);
			jitters.put(point,jitter);
		}
		return jitter;
	}
