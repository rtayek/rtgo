...
public class NamedThreadGroup { // one set of named threads.
	public static class Check {
		...
		public void startCheck() {
			if(!doCheck) return;
			if(checkManyThings) {
				initialThreads=IOs.activeThreads();
				ids=removeLeftoverThreads(initialThreads);
			}
		}
		public void howMany() {
		...
		public void endCheck() {
			...
			if(checkManyThings) {
				...
				// Logging.mainLogger.info("keyset:
				// "+NamedThreadGroup.nameToNamedThreadSet.keySet());
				howMany();
			}
		}
	...
	public class NamedThread extends Thread {
		...
		@Override public void start() {
			Logging.mainLogger.config("started: "+getName());
			super.start();
		}
		@Override public String toString() {
			...
			return this==null?null:this.getName()+", "+this.getState()+", isa: "+this.isAlive()+", iisi:  "+this.isInterrupted();
		}
		public final Runnable runnable;
		public final long groupId;
		public final int id=++ids;
	}
	public NamedThreadGroup(long groupId) {
		this.groupId=groupId;
	}
	public synchronized void addNamedThread(NamedThread namedThread) {
		namedThreadsWithRunnables.add(namedThread);
	}
	public synchronized Set<NamedThread> activeNamedThreads() {
		...
		for(NamedThread namedThread:namedThreadsWithRunnables) {
			...
			for(Thread thread:threads)
				if(namedThread.equals(thread)) {
					found=true;
					active.add(namedThread);
					break;
				}
		...
		if(inActive.size()>0) {
			// IO.stackTrace(10);
			// Logging.mainLogger.info("removing from group: "+groupId+",
			// inactive: "+inActive.size()
			// +" inactive named threads: "+inActive);
			namedThreadsWithRunnables.removeAll(inActive);
			// Logging.mainLogger.info(
			// "removing from all: "+groupId+", inactive: "+inActive.size()+"
			// inactive named threads: "+inActive);
			allNamedThreads.removeAll(inActive);
		}
		if(namedThreadsWithRunnables.size()==0); // Logging.mainLogger.info("namedThreadsWithRunnables
													// is empty!");
		return active;
	}
	...
	public int interruptNamedThreads(String string) {
		...
		for(NamedThread namedThread:activeNamedThreads())
			if(!namedThread.getState().equals(Thread.State.TERMINATED)) {
				...
				else; // Logging.mainLogger.info("int: "+namedThread.getName()+"
						// "+IO.toString(namedThread)+"
						// "+IO.toString(namedThread)+"was not terminated!");
		...
		// if(remaining>0) printNamedThreads("after remove all, "+remaining+"
		// remaining.");
		return remaining;
	}
	public static synchronized void removeAllTerminated() {
		Set<Thread> terminated=new LinkedHashSet<>();
		for(Thread thread:allNamedThreads)
			if(thread.getState().equals(State.TERMINATED)) terminated.add(thread);
	...
	public static synchronized Set<NamedThread> allActiveNamedThreads() {
		...
		for(Thread thread:threads)
			if(thread instanceof NamedThread) set.add((NamedThread)thread);
		return set;
	}
	public static synchronized int removeLeftoverThreads(Set<Thread> initialThreads) {
		...
		for(Thread thread:initialThreads)
			if(thread instanceof NamedThread) if(((NamedThread)thread).id<=ids) { // maybe
																					// bogus?
	...
	public static int checkThreads(int ids0,Set<Thread> initialThreads,Set<Thread> finalThreads,boolean quiet) {
		...
		if(extra>0) {
			...
			for(Thread thread:extraThreads)
				if(thread instanceof NamedThread) {
			...
			if(notInRange.size()>0); // Logging.mainLogger.info("not in range:
										// "+ids0+":"+NamedThreadGroup.ids+"
										// "+notInRange);
		} else if(extra<0) {
			fewerThreads.removeAll(finalThreads);
			if(verbose) {
				Logging.mainLogger.info("fewer: \n");
	...
	public static Thread findThread(String name) {
		...
		for(Thread thread:threads) {
			...
			if(startWatchdog&&thread.getName().equals(name)) {
				target=thread;
				break;
			}
		}
	...
	public static void printNamedThreads(Set<NamedThread> threads,String name,boolean printAll) {
		...
		for(Thread thread:threads) {
			...
			++i;
			if(!printAll&&i>n) {
				Logging.mainLogger.info((threads.size()-n)+" more ...");
				break;
			}
		}
	}
	public static synchronized void printNamedThreads(long groupId,String string,boolean printAll) {
		...
		if(namedThreadGroup!=null) printNamedThreads(namedThreadGroup.activeNamedThreads(),"group: "+groupId+" "+string+" activeNamedThreads()",printAll);
	...
	public static void stopAllStopables() throws InterruptedException {
		for(Long key:groupIdToNamedThreadGroup.keySet())
			stopGroup(groupIdToNamedThreadGroup.get(key),key);
	}
	public static int printNamedThreadGroups(boolean print) {
		...
		for(Long id:groupIdToNamedThreadGroup.keySet()) {
			...
			if(threads!=null&&threads.size()>0) {
				if(!once) {
					once=true;
					if(print) Logging.mainLogger.info("all threads groups:");
				}
				n+=threads.size();
				if(print) {
					System.out.println("group id: "+id+" has "+threads.size()+" threads, threadsAndRunnables: "+threads);
				}
	...
	public static void printThraedsAtEnd() {
		...
		int n;
		if((n=printNamedThreadGroups(print))>0) {
			Logging.mainLogger.info("badness2!");
		}
		if(n>0) {
			int remaining=0;
			for(Long key:groupIdToNamedThreadGroup.keySet()) {
			...
			while(printNamedThreadGroups(print)>0) {
				...
				if(++i>5) {
					Logging.mainLogger.info("giving up after "+i+" iterations.");
					break;
				}
				Logging.mainLogger.info("iteration: "+i);
			}
	...
	public static void lastPrint(Collection testsRun) {
		NamedThreadGroup.printThraedsAtEnd();
		...
		Logging.mainLogger.info("tests run: "+testsRun);
	}
	...
	// public static final Map<String,Set<NamedThread>> nameToNamedThreadSet=new
	// TreeMap<>();
