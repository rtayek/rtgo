...
public class IOs {
	...
	public static String toString(Thread thread) {
		...
		return thread==null?null
				:"thread: name: "+thread.getName()+", state: "+thread.getState()+", is alive: "+thread.isAlive()+", is interrupted:  "+thread.isInterrupted();
	}
	...
	private static void stopThread_(Thread thread) {
		if(thread.isAlive()) {
			...
			GTPBackEnd.sleep2(GTPBackEnd.yield);
			if(thread.isInterrupted()) {
				if(currentThreadIsTimeLimited()) Logging.mainLogger.info("time limited thread not joining with: "+thread);
				else {
					GTPBackEnd.sleep2(GTPBackEnd.yield);
					try {
	...
	public static void stopThread(Thread thread,Stopable stopable) {
		if(thread!=null) if(!thread.equals(Thread.currentThread())) {
			...
			if(!endState.equals(Thread.State.TERMINATED)) Logging.mainLogger.info("thread not terminated! "+thread.getName());
		} else {
			Logging.mainLogger.severe("trying to interrupting current thread!: "+thread.getName());
		}
	}
	public static /* synchronized */ void myClose(Reader in,Writer out,Socket socket,final NamedThread thread,String name,Stopable stopable) { // super.close()?
		...
		if(socket!=null) {
			if(!socket.isClosed()) try { // seems to work better here before in
											// and out.
	...
	public static void xstackTrace(int limit) {
		StackTraceElement[] elements=Thread.currentThread().getStackTrace();
		System.out.println("Printing short stack trace (top "+limit+" lines):");
		// Start index at 2 to skip getStackTrace() and printShortStackTrace()
		// method calls
		for(int i=2;i<Math.min(2+limit,elements.length);i++) {
	...
	public static void stackTrace(int n) {
		...
		System.out.print("[");
		for(int i=2;i<Math.min(n,ste.length);++i)
			System.out.print(name+" "+ste[i]+", ");
		System.out.println("]");
	}
