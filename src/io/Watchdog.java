...
public class Watchdog extends Thread {
    Watchdog(Thread threadToWatch,int time) {
        ...
        Logging.mainLogger.info("watch dog for: "+threadToWatch);
    ...
    @Override public void run() {
        ...
        Logging.mainLogger.info(this+" w started "+Init.first.et.etms());
        ...
        if(!done) {
            ...
            reason="!done.";
            Logging.mainLogger.info("interrupting: "+threadToWatch);
            threadToWatch.interrupt();
            try {
                StackTraceElement[] stackTraceElements=threadToWatch.getStackTrace();
                Logging.mainLogger.info(stackTraceElements.length+" elements.");
                //for(StackTraceElement stackTraceElement:stackTraceElements) Logging.mainLogger.info(stackTraceElement);
                threadToWatch.join(100);
                Logging.mainLogger.severe(this+" w join result: "+threadToWatch.isAlive()+" "+threadToWatch.getState());
                Logging.mainLogger.info("w run "+IOs.toString(threadToWatch));
                ...
                for(Thread thread:threads) Logging.mainLogger.info("w run 2 "+IOs.toString(thread));
            }
        } else reason="done.";
