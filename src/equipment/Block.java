...
public record Block(Stone who,List<Point> points,int liberties) {
	public Block(Board board,Point point,boolean[][] processed) {
		this(build(board,point,processed));
	}
	public Block(Board board,int x,int y,boolean[][] processed) {
		this(board,new Point(x,y),processed);
	}
	private Block(BlockData data) {
		this(data.who(),List.copyOf(data.points()),data.liberties());
	}
	public Stone color() {
		return who;
	}
	...
	public static Point modulo(Point point,Board board) { // only works for dx
															// of 1!
		...
		if(!processed[point.x][point.y]) {
			...
			if(whoOnBoard==color) {
				...
				for(Point p:neighbors)
					grow(p,board,processed,color,collected);
			}
		}
	}
	private static int count(Point point,Board board,boolean[][] processed) {
		if(!board.topology().equals(Topology.normal)) point=modulo(point,board);
		if(board.isOnBoard(point)&&!processed[point.x][point.y]) { // checking
																	// not
																	// process
																	// will
																	// unsure
	...
	private static int countLiberties(Board board,List<Point> points) {
		...
		for(int k=0;k<points.size();k++) {
			...
			for(Point point:neighbors)
				liberties+=count(point,board,processed);
		}
		return liberties;
	}
	...
	public static Pair<List<Block>,List<Block>> findBlocks(Board b) { // only
																		// used
																		// by
																		// tests
	...
	public static Block findAdjacentSelfCapturedBlock(Board board,Point point,Stone stone) {
		Point adjustedPoint=board.topology().equals(Topology.normal)?point:modulo(point,board);
		if(!board.isOnBoard(adjustedPoint)) throw new RuntimeException(adjustedPoint+" is not on boars.");
		Block block=null;
		if(board.at(adjustedPoint).equals(stone)) {
			block=find(board,point);
			if(block!=null&&block.liberties()==0) return block;
		}
		return block;
	}
	public static ArrayList<Block> findAdjacentCapturedOpponentsBlocks(Board board,Point at,Stone stone) {
		...
		for(Point point:adjusted)
			if(board.at(point).equals(stone.otherColor())) {
