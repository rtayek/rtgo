...
public class GTPBackEnd implements Stopable {
	...
	public NamedThread startGTP(long id) { // starts with a new thread each
											// time.
		if(IOs.currentThreadIsTimeLimited()) Logging.mainLogger.severe("not main! "+id+" "+model.name+" "+Thread.currentThread().getName());
	...
	private void waitForAMove(Message message) { // generate a move
		...
		Logging.mainLogger.fine(model.name+" "+"gtp: waiting for move to complete on board "+(model.moves()+1)+" to complete.");
		model.waitForMoveCompleteOnBoard(old);
		Logging.mainLogger.fine(model.name+" "+first.et+" before set waiting for move to false "+model.moves()+" to false.");
		...
		if(message.command!=null) switch(message.command) {
			...
			case boardsize:
				...
				if(message.arguments.length>1) {
					...
					if(1<=n&&n<=Model.LargestBoardSize) {
						...
						} else ModelTreeOps.setRoot(model,n,n,model.boardTopology(),model.boardShape());
						// shape and type above come from parameters through
						// model?
						send(okCharacter,message.id,"");
			...
			case play:
				...
				if(message.arguments.length>2) {
					if(model.board()==null) ModelTreeOps.setRoot(model,Board.standard,Board.standard,Topology.normal,Shape.normal);
					...
					if(who!=null) {
						if(/*!model.strict||*/who.equals(model.turn())) { // may
																			// not
																			// be
																			// needed?
			...
			case genmove:
				...
				if(message.arguments.length>1) {
					...
					if(who!=null) {
						...
						// maybe we need two stricts, one for the model and one
						// for gtp?
						if(generateMove) { // generate move. can return "PASS"
											// "Resign"
	...
	public boolean getGenerateMove() {
		return generateMove;
	}
	...
	@Override public void run() {
		try { // this reads one line and processes it.
			...
			loop:for(String string=in.readLine();string!=null;string=in.readLine()) {
				...
				if(done) {
					Logging.mainLogger.fine(model.name+"done in run loop.");
					break loop;
				}
				if(namedThread!=null&&namedThread.isInterrupted()) {
				...
				try {
					...
					if(isStopping) Logging.mainLogger.info("stopping - process command caught: "+e);
						...
						// break for now, but maybe try to continue/recover
						// later
					}
					break loop;
				}
				// Logging.mainLogger.info(model.name+" after process command:
				// "+string+" "+isWaitingForMove());
	...
	public void waitForExitRunLoop() {
		while(!done)
			GTPBackEnd.sleep2(GTPBackEnd.yield);
	}
	...
	public String runCommands(boolean justRun) {
		runBackend(justRun);
		String string=out.toString(); // hackish, in this case out is a string
										// writer!
