...
public class GameFixture implements Stopable {
	public enum States {
		starting,waitForBlackMove,waitForWhiteMove,ended,
	}
	...
	public void startGameThread() {
		// IO.stackTrace(10);
		if(!backends) {
			Logging.mainLogger.info("starting backends.");
			startPlayerBackends();
		}
		if(!initialized) {
			Logging.mainLogger.info("initializing game.");
		...
		if(namedThread!=null) throw new RuntimeException("game thread alreasy exists!");
			...
			Logging.mainLogger.info("starting gam.e");
	...
	String generateMove() {
		...
		switch(recorderFixture.backEnd.model.turn()) {
			case black:
				...
				if(response==null) {
					mainLogger.severe("genmove new behavior black");
					return null;
				}
			...
			case white:
				...
				if(response==null) {
					mainLogger.severe("genmove new behavior white");
					return null;
				}
				if(response.isBad()) {
					mainLogger.severe("white bad oops");
					mainLogger.warning(response.toString());
				}
				return response.response;
			default:
	...
	@Override public boolean setIsStopping() {
		boolean rc=isStopping;
		isStopping=true;
		return rc;
	}
	public void printHistograms(double dt) {
		if(dt!=0) {
			Logging.mainLogger.info("game took "+dt+" ms.");
			double speed=recorderFixture.backEnd.model.moves()/et.etms();
			Logging.mainLogger.info("speed: "+speed+" ,moves/ms.");
			Logging.mainLogger.info("average time: "+(1/speed)+" ms. for one move.");
		}
		Logging.mainLogger.info("histograms");
		Logging.mainLogger.info("recorder "+recorderFixture.backEnd.model.histogram);
		Logging.mainLogger.info("   black "+blackFixture.backEnd.model.histogram);
		if(whiteFixture.backEnd!=null) Logging.mainLogger.info("   white "+whiteFixture.backEnd.model.histogram);
		Logging.mainLogger.info("before: "+waitBefore);
		Logging.mainLogger.info("after: "+waitAfter);
	}
	@Override public void stop() { // change name to stop back ends
		isStopping=true;
		if(recorderFixture!=null) {
			recorderFixture.stop();
			GTPBackEnd.sleep2(longSleepTime);
		}
		if(blackFixture!=null) {
			blackFixture.stop();
			GTPBackEnd.sleep2(longSleepTime);
		}
		if(whiteFixture!=null) {
			whiteFixture.stop();
			GTPBackEnd.sleep2(longSleepTime);
		}
	...
	public Response initializeGame() { // requires back end threads to be
										// running
		initialized=true;
		if(namedThread!=null) {
			Logging.mainLogger.info("game already started 2");
			System.exit(0);
		}
		...
		if(recorderFixture.backEnd.namedThread==null) {
			Logging.mainLogger.info("recorder mot started!");
			System.exit(0);
		}
		...
		if(recorderFixture.backEnd.model.board()!=null) {
			...
			if(blackFixture.backEnd.namedThread==null) {
				Logging.mainLogger.info("black backend not started!");
				System.exit(0);
			}
			...
			if(whiteFixture.backEnd.namedThread==null) {
				Logging.mainLogger.info("white backend not started!");
				System.exit(0);
			}
			response=whiteFixture.frontEnd.sendAndReceive(Command.tgo_white.name());
	...
	@Override public void run() {
		try {
			...
			while(!isStopping) {
				...
				String command,move;
				gameLogger.info("get "+recorderFixture.backEnd.model.turn()+" move #"+(recorderFixture.backEnd.model.moves()+1));
				move=generateMove();
				...
				gameLogger.info("recorded move #"+recorderFixture.backEnd.model.moves()+"@"+recorderFixture.backEnd.model.lastMoveGTP());
				if(saveGameAfterMove) {
					Logging.mainLogger.info("saving game.");
					try {
						...
						boolean wasSaved=ModelTrees.saveModel(recorderFixture.backEnd.model,writer);
						writer.close();
						if(!wasSaved) {
							gameLogger.warning("save game failed!");
							Logging.mainLogger.info("game save failed!.");
						} else Logging.mainLogger.info("game saved.");
					} catch(Exception e) {
						Logging.mainLogger.info("save game caught: "+e);
					}
				}
				// maybe. if this were a correspondence game, we should be able
				// to restore from this point.
				// yes, how to restore from here?
				// write a test for the above
				// shove will probably work.
				switch(recorderFixture.backEnd.model.turn()) { // send move to
																// the other
																// player
					// these are different, they need to be the same!
					case black:
						command=Command.play.name()+" "+"white"+" "+move; // may
																			// get
																			// confuses
																			// if
																			// we
				...
				if(move.contains(gtpResignString)) {
					gameLogger.info("resign: "+move+" "+response);
					break;
				}
				if(recorderFixture.backEnd.model.moves()>maxMoves) {
					gameLogger.info("breaking out of game run loop after "+recorderFixture.backEnd.model.moves()+" moves.");
					break;
				}
			}
	...
	public void printThreads() {
		Logging.mainLogger.info("2 game thread: "+namedThread);
		Logging.mainLogger.info("recorder: "+recorderFixture.backEnd.namedThread);
		if(blackFixture.backEnd!=null) Logging.mainLogger.info("   black: "+blackFixture.backEnd.namedThread);
		if(whiteFixture.backEnd!=null) Logging.mainLogger.info("   white: "+whiteFixture.backEnd.namedThread);
	}
	public void playOneMove(BothEnds player,Move2 move) { // does not wait for
															// opponent's board.
		double t0=et.etms();
	...
	public void printStatus() {
		Logging.mainLogger.info("backends: "+backends+", initialized: "+initialized+", started: "+started);
	}
	...
	private void waitForGameThread() {
		if(namedThread!=null) return;
		long t0=System.nanoTime();
		long timeoutNanos=5_000_000_000L;
		while(namedThread==null&&!isStopping) {
			GTPBackEnd.sleep2(GTPBackEnd.yield);
			if(System.nanoTime()-t0>timeoutNanos) break;
		}
	...
	public int playOneMoveAndWait(BothEnds player,BothEnds opponent,Move2 move) {
		...
		if(move.equals(Move2.nullMove)) throw new RuntimeException(move+" 4 "+playerModel.lastMoveGTP()+" "+playerModel.lastMove2()+" move oops");
		...
		double t0=et.etms();
		// if(move instanceof Resign||move instanceof Pass) throw new
		// RuntimeException(move+" before wait");
		...
		if(!areBoardsEqual()) {
			// Logging.mainLogger.info(recorderFixture.backEnd.model);
			// Logging.mainLogger.info(blackFixture.backEnd.model);
			// Logging.mainLogger.info(whiteFixture.backEnd.model);
	...
	public static void playSillyGame(GameFixture gameFixture,int m,boolean verbose) throws InterruptedException {
		...
		// check this!
		// seems like this is simulating two players playing omn their own
		// boards.
		...
		for(int i=0;i<m;++i) {
			...
			int moves=turn==Stone.black?gameFixture.playOneMoveAndWait(black,white,move):gameFixture.playOneMoveAndWait(white,black,move);
			// histogram.add(et.etms());
			sleep2(GTPBackEnd.yield);
			// if(i==m-1) { Logging.mainLogger.fine("odd number of moves");
			// oddNumberOfMoves=true; break; }
		}
		Model recorder=gameFixture.recorderFixture.backEnd.model;
		// sleep2(5);
		// while(recorder.moves()<m) sleep2(GTPBackEnd.yield);
		if(recorder.moves()!=m) {
			Logging.mainLogger.info(recorder.moves()+"!="+m);
		...
		if(verbose) {
			...
			Logging.mainLogger.info("black: "+blocks.first.size()+" blocks (0).");
			Logging.mainLogger.info("white: "+blocks.second.size()+" blocks (180).");
			Logging.mainLogger.info("black has "+recorder.prisoners(Stone.white)+" white prisoners");
			Logging.mainLogger.info("white has "+recorder.prisoners(Stone.black)+" black prisoners");
	...
	public static void printStuff(GameFixture game) {
		...
		Logging.mainLogger.info("3 waitstates: "+blackMoves+" black moves "+isBlackWaitingForAMove+", "+whiteMoves+" white moves "+isWhiteWaitingForAMove);
	}
